...
// MigrateRefTags rewrites legacy " @<id>" project/story ref tags to the
// canonical " #<id>" form across all data files (daily, weekly, backlog,
// recycle bin, recurring templates). It is safe to call on every startup:
// when no files carry the old format the function returns immediately without
// touching any file (idempotent). When files do need rewriting, the relevant
// data files are backed up to <dataDir>/.pre-hashtag-backup first (the backup
// directory is created at most once; later runs that find nothing to migrate
// skip the backup step). Files that cannot be read are logged and skipped;
// files that cannot be written cause MigrateRefTags to return an error.
func (s *Store) MigrateRefTags() error {
	...
	if _, serr := os.Stat(backupDir); os.IsNotExist(serr) {
		if berr := backupFiles(s.DataDir, backupDir, paths); berr != nil {
	...
	for _, p := range toMigrate {
		if rerr := migrateFile(p, known); rerr != nil {
...
// migrateFilePaths returns the paths of all data files that may carry ref tags:
// daily files, weekly files, backlog, recycle bin, and recurring templates.
// Missing files are silently omitted.
func (s *Store) migrateFilePaths() ([]string, error) {
	var paths []string
...
// migrateFile rewrites path in place, replacing each trailing " @<known_id>"
// with " #<known_id>". Uses an atomic write (write-to-tmp, rename) so the
// file is never left in a partial state.
func migrateFile(path string, known map[string]bool) error {
	data, err := os.ReadFile(path)
	if err != nil {
...
// a single text line. It mirrors the splitStoryTag logic: only the very last
...
// backupFiles copies each file in paths (rooted under src) to the corresponding
// path under dst, creating parent directories as needed. Files that cannot be
// read are skipped with a warning; an error creating a backup destination is fatal.
func backupFiles(src, dst string, paths []string) error {
